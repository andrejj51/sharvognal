...
function awardsPage() {
  const p=currentUser()?player(currentUser().player_id):null;
  const note=p?`Ваша коллекция: ${earnedFor(p.id).length} из ${state.awards.length} открыто. <a href="#player/${p.id}">Мой профиль →</a>`:'Имена и изображения закрытых наград скрыты. Прогресс вашей коллекции — в профиле игрока.';
  return `<div class="page-head"><div><div class="eyebrow">Коллекция Зазеркалья</div><h1>Мемные трофеи</h1><p>Награды открываются только при 100% прогресса.</p></div><button class="button ghost" data-action="settings">Настроить награды</button></div><p class="section-note">${note}</p>${collections(p)}`;
}
function render() {
  if(!state) return;
  ...
  else if(route[0]==='awards') app.innerHTML=awardsPage();
